...
namespace {
    ...
    void TestPrint() {
        ...
        sheet->SetCell("A4"_pos, "rrrr");
        ASSERT_EQUAL(sheet->GetPrintableSize(), (Size{ 4, 1 }));
        {
            std::ostringstream texts;
            sheet->PrintTexts(texts);
            ASSERT_EQUAL(texts.str(), "\nmeow\n\nrrrr\n");
    ...
    void TestCellCircularReferences() {
        ...
        ASSERT(caught);
        std::cout << sheet->GetCell("M6"_pos)->GetText() << std::endl;
